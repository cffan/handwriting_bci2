...
def prepareDataCubesForRNN(sentenceFile, singleLetterFile, labelFile, cvPartitionFile, sessionName, rnnBinSize, nTimeSteps, isTraining,
    ...
    for t in range(labelsDat['timeBinsPerSentence'].shape[0]):
        ...
        charStart = np.argwhere(charStartTarget[t, :] > 0.5)
        errWeights[t, 0:charStart[0,0]] = 1

        #Also, we cut off the end of the trial if there is a very long pause after the last letter - this could hurt
        #training.
        maxPause = 150
        errWeights[t,(charStart[-1,0]+maxPause):] = 0
        labelsDat['timeBinsPerSentence'][t,0] = (charStart[-1,0]+maxPause)
    ...
    if isTraining:
        ...
        padNeuralData = neuralData
        padCombinedTargets = combinedTargets
        padErrWeights = errWeights
    else:
