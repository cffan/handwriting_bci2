...
def generateCharacterSequences(args):
    ...
    synthNeural, synthCharProb, synthCharStart, synthMask, synthSentence, synthSentenceLength = \
        makeSyntheticDataFromRawSnippets(charDef=args['charDef'],
    ...
    if args['binSize']==1:
        ...
        synthMaskBinned = synthMask
    else:
        S, T, N = synthNeural.shape
        binSize = args['binSize']
        synthNeuralBinned = np.mean(np.reshape(synthNeural, [S, T // binSize, binSize, -1]), axis=2)
        synthTargetsBinned = np.mean(np.reshape(synthTargets, [S, T // binSize, binSize, -1]), axis=2)
        synthMaskBinned = (np.mean(np.reshape(synthMask, [S, T // binSize, binSize]), axis=2) > 0.5).astype(np.float32)
    ...
    for trialIdx in range(args['nSentences']):
        feature = {'inputFeatures': _floats_feature(np.ravel(synthNeuralBinned[trialIdx,:,:]).tolist()),
            # The following three are not defined for now
            'classLabelsOneHot': _floats_feature(np.ravel(np.zeros_like(synthTargetsBinned)[trialIdx,:,:]).tolist()),
            'ceMask': _floats_feature(np.ravel(np.zeros_like(synthNeuralBinned)[trialIdx,:, 0]).tolist()),
            'newClassSignal': _floats_feature(np.ravel(np.zeros_like(synthNeuralBinned)[trialIdx,:,:1]).tolist()),
            'seqClassIDs': _ints_feature(synthSentence[trialIdx]),
            'nTimeSteps': _ints_feature([synthNeuralBinned.shape[1]]),
            'nSeqElements': _ints_feature([synthSentenceLength[trialIdx]]),
            'transcription': _ints_feature(synthSentence[trialIdx]),
        }
    ...
    #return synthNeuralBinned, synthTargetsBinned, synthMaskBinned

def makeSyntheticDataFromRawSnippets(charDef, charSnippets, nSentences, nSteps, wordList, blankProb=0.05,
                                     accountForPenState=True, rareLetterIncrease=False, rareWordList=[],
                                     maxSeqLen=500):
    """
    ...
    synthMask = np.zeros([nSentences, nSteps])
    synthSentence = np.zeros([nSentences, maxSeqLen], dtype=np.int32)
    synthSentenceLength = np.zeros([nSentences], dtype=np.int32)
    ...
    for t in range(nSentences):
        ...
        randomNSteps = np.random.randint(2400, nSteps)
        while currIdx<randomNSteps:
            ...
            # Plus 1 because the RNN training converts this to a sparse tensor which treats
            # 0 as an empty value.
            sentence.append(charDef['charListAbbr'].index(currentWord[currentLetterIdx]) + 1)

            #fill in the data tensors for this character
            nNewSteps = currentSnippet.shape[0]
            if nNewSteps+currIdx >= randomNSteps:
                stepLimit = randomNSteps - currIdx
            ...
            synthMask[t, currIdx:(currIdx+currentSnippet.shape[0])] = 1

            #advance pointer to the next character
            currIdx += nNewSteps
            currentLetterIdx += 1

        synthSentence[t, :len(sentence)] = np.array(sentence)
        synthSentenceLength[t] = len(sentence)


    return synthNeural, synthCharProb, synthCharStart, synthMask, synthSentence, synthSentenceLength
